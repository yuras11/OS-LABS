...
void Matrix::MultipleBlocksOfMatrix(const Matrix &matrix1, const Matrix &matrix2, Matrix &result, int blockI, int blockJ, int matrixSize, int blockSize)
{
...
void Matrix::MultiplyWithThreads(Matrix &matrix1, Matrix &matrix2, Matrix &result, int matrixSize, int blockSize)
{
    ...
    for (int blockI = 0; blockI < matrixSize; blockI += blockSize)
    {
        ...
        {
            mutex.lock();
            threads.emplace_back(Matrix::MultipleBlocksOfMatrix, std::cref(matrix1), std::cref(matrix2), std::ref(result), blockI, blockJ, matrixSize, blockSize);
            mutex.unlock();
        }
    }
